...
public:
    struct IntervalResult {
        ...
        IntervalResult(int l, int r, const std::vector<std::shared_ptr<std::vector<unsigned char>>>& t)
            : left(l), right(r), tokens(t) {}
    };
...
protected:
    ...
    static constexpr double THRESHOLD = 0.1; // hamming 距离阈值
...
public:
    ...
    void update(int id, const std::shared_ptr<std::vector<unsigned char>>& token, const std::string& keyword);
    std::vector<std::shared_ptr<std::vector<unsigned char>>> query(int l, int r);
    std::vector<IntervalResult> getCandidateIntervals(int l, int r, const std::string& keyword);
