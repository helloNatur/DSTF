...
protected:
    void SetUp() override {
        ...
        keyword1 = "keyword0table1_keyword_0_0";

        constexpr int key_colnum = 9;
        constexpr int join_column = 1;
        record_num = 5000; // 65536
        ...
        id_tokens1 = table_1.getCandidateIntervals(id_start, id_end,std::string{keyword1});
    }
...
TEST_F(IdTokenTest, ConstructorAndInitialization) {
    ...
    EXPECT_EQ(segment_tree->get_size(), record_num)<< "SegmentTree size should be 65536"; // Assuming size is 65536
    EXPECT_GE(segment_tree->get_tree().size(), 4 * record_num)<< "SegmentTree size should be at least 4 * n"; // Tree size should be at least 4 * n
}

TEST_F(IdTokenTest, IdTokensSizeAndContent) {
    ASSERT_FALSE(id_tokens1.empty()) << "id_tokens1 should not be empty";
    EXPECT_GE(id_tokens1.size(), 1000) << "id_tokens1 size should be at least 1000";
    
    int i=0;
    for(int idx=0; idx < id_tokens1.size(); ++idx) { // 迭代 id_tokens1 中的每个区间
        const auto& interval = id_tokens1[idx]; // 获取当前区间
        for (const auto& token : interval.tokens) { // 迭代 tokens 中的每个 token
            //初始化阶段在0-2000之间的所有id_tokens1
            std::string keyword0 = "idtable1_id_" + std::to_string(interval.left);
...
TEST_F(IdTokenTest, SearchWithIdTokens) {
    ...
    std::ifstream file("/nvme/baum/git-project/JXT2/data/pg_0-2000.csv");
    if (!file.is_open()) {
