...
void SegmentTree::update(size_t node, int start, int end, int id, 
    ...
    int mid = (start + end) / 2;
    if (id <= mid)
        update(2 * node, start, mid, id, token, keywords);
    else
        update(2 * node + 1, mid + 1, end, id, token, keywords);
...
// 输入：区间以及关键词
// 输出：包含关键词的叶子结点区间和对应的tokens
// 内部节点只用于剪枝：如果内部节点的BF说“没有”，则可以安全地剪掉整个分支，不再向下递归。
// 结果只在叶子节点产生：只有当递归到达叶子节点，并且该叶子节点的BF也说“有”时，才将该叶子节点的信息加入结果集。
void SegmentTree::getCandidateIntervals(size_t node, int start, int end, 
    ...
    // 检查 bf 是否有效
    // 2:bf剪枝：检查当前节点的BF
    if (!tree[node].bf) {//空指针跳过
        return; // 如果 bf 为 nullptr，跳过
    }
    bool match_in_bf = false;
    for (const auto& keyword : keywords) {
        if (tree[node].bf->lookup(keyword)) {  //这里发生段错误：空指针
            match_in_bf = true;
            break;
        }
    }

    // 如果当前节点的 Bloom Filter 不包含关键词
    // 那么它的所有子节点也必然不匹配，直接剪掉整个分支。
    if (!match_in_bf) return;

    // 3: 如果是叶子节点，直接添加 tokens
    ...
    //4:如果是内部节点，则继续向下递归
    // int mid = (start + end) / 2;
    int mid = start + (end - start) / 2; // 使用防溢出的写法
    getCandidateIntervals(2 * node, start, mid, l, r, keywords, result);
    getCandidateIntervals(2 * node + 1, mid + 1, end, l, r, keywords, result);
}

std::vector<SegmentTree::IntervalResult> 
SegmentTree::getCandidateIntervals(int l, int r, const std::vector<std::string>& keywords) {
    std::vector<IntervalResult> result;
    getCandidateIntervals(1, 0, n - 1, l, r, keywords, result);
    return result;
}