...
// Helper function to compare string_key* by content
inline bool string_key_compare(string_key* key1, string_key* key2) {
    if (key1 == key2) return true;  // Same pointer
    if (key1 == nullptr || key2 == nullptr) return false;
    if (key1->length != key2->length) return false;
    return memcmp(key1->key, key2->key, key1->length) == 0;
}

// Helper function to compare keys (supports both pointer and content comparison)
template<class T>
inline bool key_equal(T key1, T key2) {
    return key1 == key2;
}

// Specialization for string_key* to compare by content
template<>
inline bool key_equal<string_key*>(string_key* key1, string_key* key2) {
    return string_key_compare(key1, key2);
}
...
// support variable length key
    ...
    // Specialization for string_key* to hash by content instead of pointer address
    template<>
    inline uint64_t hash<string_key*>(string_key* key) {
    ...
    public:
        ...
        static Value_t lookup(T key, uint64_t hash, bucket_t *first_bucket,
            ...
            while (mask) {
                ...
                if (key_equal(key, this_entry->key)) {
