...
std::vector<SegmentTree::IntervalResult> query_hash_st_bf(
    const AdaptiveTimeBucketBuilder& time_index,
    const std::vector<std::shared_ptr<SegmentTree>>& bucket_segment_trees,
...
class SpatiotemporalDB_HashST_Test : public ::testing::Test {
protected:
    void SetUp() override {
        ...
        auto auto_bounds = BoundsForPoints(all_points_);
        ...
        int cube_code_level = GetEnvInt("JXT2_CUBE_LEVEL", 8);
        ...
        std::cout << "Building AdaptiveBucket / SegmentTree / BF index..." << std::endl;
        ...
        for (const auto& point : all_points_) {
            ...
            std::string time_key = "bucket_" + std::to_string(bucket_id) + "_slot_" +
            ...
            std::string token_input = K_token + time_key;
            ...
            bucket_segment_trees[static_cast<std::size_t>(bucket_id)]->update_deferred(
                local_slot, token, codes);
        }

        std::cout << "[SegmentTree] Finalizing deferred Bloom Filters..." << std::endl;
        ...
        std::cout << "[SegmentTree] Finalize complete: "
                  << finalize_time.count() << " ms" << std::endl;
...
TEST_F(SpatiotemporalDB_HashST_Test, PerformSpatiotemporalQueryAndVerify) {
    std::cout << "\n--- Hash ST/BF Spatiotemporal Query Test(" << query_runs
              << " runs) ---\n" << std::endl;
    ...
    if (!query_plan_path.empty()) {
        ...
        for (const auto& row : rows) {
            ...
            auto interval_results = query_hash_st_bf(time_builder, bucket_segment_trees, cube_code,
            ...
            std::vector<std::shared_ptr<std::vector<unsigned char>>> raw_tokens;
            std::size_t capacity = 0;
            for (const auto& interval_result : interval_results) {
                capacity += interval_result.tokens.size();
            }
            raw_tokens.reserve(capacity);

            std::unordered_set<std::string> seen;
            seen.reserve(capacity * 2 + 1);
            for (const auto& interval_result : interval_results) {
                for (const auto& tok_ptr : interval_result.tokens) {
                    ...
                    if (seen.insert(key).second) {
                        raw_tokens.push_back(tok_ptr);
                    }
                }
            }

            std::vector<Ciphertext> time_tokens;
            time_tokens.reserve(raw_tokens.size());
            for (const auto& ptr : raw_tokens) {
                time_tokens.push_back(*ptr);
            }
            ...
            std::set<int> candidate_ids(ids_time.begin(), ids_time.end());
            ...
            EXPECT_TRUE(std::includes(candidate_ids.begin(), candidate_ids.end(),
                ...
                << "Hash ST/BF query-plan result does not fully include ground truth";
    ...
    for (int run = 1; run <= query_runs; ++run) {
        ...
        EXPECT_TRUE(std::includes(set_time.begin(), set_time.end(),
                                  ground_truth_ids.begin(), ground_truth_ids.end()))
            << "Hash ST/BF result does not fully include ground truth";
