...
TEST_F(SpatiotemporalDB_Test, PerformanceBenchmark) {
    ...
    if (!query_plan_path.empty()) {
        ...
        for (const auto& row : rows) {
            ...
            const auto rangequery = db_->benchmarkRangeQuery(grid_rect);
            const double total_ms = rangequery.timings.query_gen_ms +
                                    rangequery.timings.eval_ms +
                                    rangequery.timings.result_decrypt_ms;

            std::set<int> candidate_ids(rangequery.set_times.begin(),
                                        rangequery.set_times.end());
            ...
            EXPECT_TRUE(std::includes(candidate_ids.begin(), candidate_ids.end(),
                ...
                << "Tdag-SRC query-plan result does not fully include ground truth";
            ...
            PrintQueryPlanResult("Tdag-SRC", row,
                                 rangequery.timings.query_gen_ms,
                                 rangequery.timings.eval_ms,
                                 rangequery.timings.result_decrypt_ms,
                                 total_ms,
                                 candidate_ids.size(),
