...
class I1HashTdagBFTest : public ::testing::Test {
protected:
    void SetUp() override {
        auto start_build = std::chrono::high_resolution_clock::now();

        day_index = std::make_unique<PeaDramDayIndex>();
        ...
        for (const auto& [day_ts, entries] : entries_by_day) {
            ...
            index->build(entries, keyword_map, "hash_i1_day_" + std::to_string(day_ts));
            auto* index_ptr = index.get();
            day_indexes.emplace(day_ts, std::move(index));
            day_index->insert(day_ts, index_ptr);
        }
        ...
        std::cout << "[I1] Day Buckets: " << day_index->size() << std::endl;
    ...
    std::unique_ptr<PeaDramDayIndex> day_index;
...
TEST_F(I1HashTdagBFTest, PerformSpatiotemporalQueryAndVerify) {
    std::cout << "\n--- PeaHash Logarithmic-SRC-i1 TDAG/BF Query Test("
    ...
    for (int run = 1; run <= query_runs; ++run) {
        ...
        auto* day = day_index->firstIntersectingDay(start_day);
        ...
        while (day != nullptr && day->day_ts <= end_day) {
            auto* index = static_cast<LogarithmicSrcI1Index*>(day->root);
            if (index != nullptr) {
                int sh = (day->day_ts == start_day) ? start_interval : 0;
                int eh = (day->day_ts == end_day) ? end_interval : 143;
                auto day_ids = I1RunQuery(*index, *emm_engine, sh, eh, timings);
